...
// Reusable component used in both the checkout card tab and cart drawer.
// Uses Stripe PaymentRequestButtonElement — shows automatically based on browser.
// Returns null if browser does not support Apple Pay or Google Pay (no empty space).
...
export interface ExpressCheckoutProps {
  orderTotal: number;
  orderNumber: string;
  /** Optional — Apple/Google Pay provides this from the payment sheet */
  ...
  /** When true, shows the OR divider below the button */
  showDivider?: boolean;
}
...
function InnerExpressCheckout({
  ...
  useEffect(() => {
    ...
    pr.on('paymentmethod', async (ev) => {
      // Use payer info from Apple/Google Pay sheet if not provided
  ...
  // Returns null if browser doesn't support Apple Pay or Google Pay — no empty space
