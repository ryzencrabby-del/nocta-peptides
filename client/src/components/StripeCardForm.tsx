...
function InnerCardForm({
    ...
    pr.on('paymentmethod', async (ev: any) => {
      try {
        const response = await fetch('/api/create-payment-intent', {
          ...
          body: JSON.stringify({ orderTotal, orderNumber, customerEmail: ev.payerEmail || customerEmail, customerName: ev.payerName || customerName, shippingAddress, items }),
        });
        const { clientSecret, error: serverError } = await response.json();
        if (serverError || !clientSecret) { ev.complete('fail'); return; }
  ...
  const handleStripePayment = async () => {
    ...
    if (!nameOnCard.trim()) { setErrorMessage('Please enter the name on your card.'); return; }
    setIsLoading(true);
    ...
    try {
      ...
      const result = await stripe.confirmCardPayment(clientSecret, {
        payment_method: { card: cardElement, billing_details: { name: nameOnCard, email: customerEmail } },
      });
      if (result.error) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="px-3 py-3 border border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-[#1A3A4A] transition-all">
          <CardElement
            options={{
              style: {
                base: { fontSize: '14px', color: '#1A3A4A', fontFamily: 'Inter, sans-serif', '::placeholder': { color: '#9ca3af' } },
      ...
      >
        {isLoading ? (
          <><Loader2 size={16} className="animate-spin" />{loadingMessage || 'Processing...'}</>
        ) : (
          <><CreditCard size={16} />Pay Now — ${orderTotal.toFixed(2)}</>
        )}
      </button>
      <p className="text-center text-xs text-gray-400">🔒 Secured by Stripe. Your card details are never stored on our servers.</p>
    </div>
  );
}
